...
HC4067 mux1(S0, S1, S2, S3, 3); // 

int Mux_Out = A4;
int hallMeasure;
int hall_min = 450; // 486 measured on hall board with power supply 5.00V
int hall_max = 600; // 584 measured on hall board with power supply 5.00V
...
void setup() {
  ...
  Serial.begin(9600);
  Serial.println("Understand_Matrix_Mux_V01");
  ...
  for (int i = 7; i >= 0; i--) {
    ...
    for (int j = 0; j < 8; j++) {
      ...
      if (hall_sensor_status[i][j] == 1 || hall_sensor_status[i][j] == -1) {
        lc.setLed(0, i, 7-j, true); // lcd matrix column not folow the same logic 
      }
      Serial.print(" ");
    }
    Serial.println('|');
